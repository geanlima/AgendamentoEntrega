...
import { ConfigTabelaPopupComponent, OfertaPopUpComponent, ValidadePopupComponent } from '@core/components';
import { ProgressComponent } from '@shared/components';
import { TypeToast } from '@shared/enums';
import { Filtro, Produto, DadoComerciais } from '@shared/models';
import { ProdutoService, StorageService, DialogService, NotificationService } from '@shared/services';
...
export class ProdutoConsultaComponent {
  ...
  private readonly VIEW_CODE: string = 'VIEW_CONSULTA_PRODUTOS';
  ...
  private idRegiao = 0;
  private acrescimoDescontoMaximo = -1;
  ...
  private loadProdutos(filtro: Filtro): void {
    if (filtro.codigoRegiao != this.idRegiao || filtro.acrescimo_desconto_maximo != this.acrescimoDescontoMaximo) {

      const progress = this.dialogService.showProgress(ProgressComponent);

      const codigoUnidade = environment.unidade;
      const codigoCliente = 0;
      this.idRegiao = filtro.codigoRegiao;
      this.acrescimoDescontoMaximo = filtro.acrescimo_desconto_maximo;

      this.produtoService.getProdutos(
        codigoUnidade,
        codigoCliente,
        +this.acrescimoDescontoMaximo,
        +this.idRegiao
      ).pipe(
        finalize(() => {
        ...
        .subscribe((produtos) => {
          ...
          this.setDataSource(this.Produtos);

          if (this.Produtos) {
            this.dataSource.sort = this.sort;
            this.dataSource.paginator = this.paginator;
            this.onFilterChanged(filtro);
          }
  ...
  exportToPDF() {
    ...
    this.ProdutosPDF.forEach(produto => {
      const precoFormatado = produto.dados_comerciais.precoTabela.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
      const semImpostoFormatado = produto.dados_comerciais.precoSemImposto.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
      ...
      linhas.push([
        ...
        produto.quantidade_embalagem_venda.toString(),
        produto.estoque_disponivel.toString()
      ]);
    });
    ...
    doc.save(`produtos_${environment.cliente.toLowerCase()}.pdf`);


  }

  isDataSourceEmpty(): boolean {
    return this.dataSource == null || this.dataSource.data.length === 0;
  }
  ...
  onLimparFiltros(filtro: Filtro): void {
    this.loadProdutos(filtro);
  }


  onPesquisar(filtro: Filtro): void {
    if (filtro.codigoRegiao === undefined || filtro.codigoRegiao === 0 || filtro.codigoPlanoPagamento === undefined || filtro.codigoPlanoPagamento === 0) {
      this.notificationService.showToast(
        {
          message: "Selecione a região e o plano de pagamento para realizar a pesquisa",
  ...
  onFilterChanged(filtro: Filtro): void {
    ...
    this.ProdutosPDF = this.dataSource.filteredData;
  }
    ...
    const modal = this.dialogService.show(OfertaPopUpComponent,
      {
        ...
        codigoCliente: 0,
        codigoRegiao: this.filter.codigoRegiao,
        ufCliente: '0',
        codigoAtividade: 0,
        codigoRede: 0,
        codigoRCA: usuario.rca_code,
        codigoSupervisor: usuario.supervisor_id,
        codigoPlanoPagamento: this.filter.codigoPlanoPagamento
      },
      []
    );
